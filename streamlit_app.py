...
with st.expander('Кластеризация методом k-means++'):
  ...
  if elbow_method_need == "Да":
    ...
    """функция, которая поможет пользователю выбрать оптимальное количество кластеров. На вход принимает только таблицу (дата-фрейм)
    и максимальное количество кластеров, указанное пользователем"""
